    ...
    <div v-if="product" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div class="flex flex-col lg:flex-row gap-12">
              ...
              <div ref="sliderRef" class="flex flex-nowrap gap-4 overflow-x-auto pb-2 pt-2 snap-x scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <button 
                  ...
                  :class="[
                    ...
                    activeImage === img ? 'border-blue-500 opacity-100 shadow-md bg-white scale-100' : 'border-transparent opacity-70 hover:opacity-100 bg-slate-100 scale-95 hover:scale-100 hover:shadow-sm'
                  ]"
                >
        ...
        <div class="w-full lg:w-1/2 flex flex-col pt-2">
          <div class="text-sm font-bold text-blue-500 uppercase tracking-widest mb-2 opacity-90">{{ product.category }}</div>
          ...
          <div class="mb-8">
            ...
            <div class="flex flex-wrap gap-2">
              <span v-for="tag in product.tags" :key="tag" class="px-4 py-2 bg-gray-50 border border-gray-200 text-gray-700 text-sm font-semibold rounded-lg hover:border-blue-300 transition-colors cursor-default">
                {{ tag }}
              </span>
            </div>
          </div>
          ...
          <div class="mts-auto">
            <div v-if="product.inStock" class="flex flex-col sm:flex-row gap-4 mb-6">
              <div class="flex items-center justify-between border-2 border-gray-200 rounded-xl p-1 bg-white h-14 w-full sm:w-36 shrink-0 transition-colors hover:border-gray-300 focus-within:border-blue-500">
              ...
              <button @click="addToCart" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center hover:scale-[1.02] active:scale-95 h-14 text-lg">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-100">
                <Truck class="w-6 h-6 text-blue-500 mr-3 shrink-0" />
                <span class="text-sm font-medium text-gray-700">Free Shipping over ₱200</span>
              </div>
              <div class="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-100">
                <ShieldCheck class="w-6 h-6 text-blue-500 mr-3 shrink-0" />
    ...
    <div v-if="product" class="border-t border-gray-100 bg-gray-50 mt-12 overflow-hidden">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 flex flex-col items-center">
          ...
          <div class="w-16 h-1.5 bg-gradient-to-r from-blue-500 to-blue-600 mx-auto rounded-full"></div>
        </div>
            ...
            <router-link :to="`/product/${rec.id}`" class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col h-[340px] block">
              ...
              <div class="p-6 flex flex-col flex-1 justify-between">
                ...
                <div class="flex items-center justify-between mt-4">
                  <p class="font-extrabold text-blue-500 text-xl">₱{{ rec.price.toFixed(2) }}</p>
                  <span class="w-8 h-8 rounded-full bg-orange-50 text-blue-500 flex items-center justify-center group-hover:bg-blue-500 group-hover:text-white transition-colors">
