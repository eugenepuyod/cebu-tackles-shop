      ...
      <div class="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="hidden md:flex bg-gray-100 rounded-lg p-1">
            <button 
              @click="viewMode = 'grid'" 
              :class="['p-2 rounded-md transition-colors', viewMode === 'grid' ? 'bg-white shadow-sm text-blue-500' : 'text-gray-500 hover:text-gray-900']"
            >
              <Grid class="w-6 h-6" />
            </button>
            <button 
              @click="viewMode = 'list'"
              :class="['p-2 rounded-md transition-colors', viewMode === 'list' ? 'bg-white shadow-sm text-blue-500' : 'text-gray-500 hover:text-gray-900']"
            >
              <List class="w-6 h-6" />
            </button>
          </div>
        ...
        <div class="block md:hidden mb-8 pl-1">
          ...
          <div class="relative">
            <input v-model="searchQuery" type="text" placeholder="Search gear..." class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm group">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 flex flex-col md:flex-row gap-8 items-start">
      ...
      >
          ...
          <ul class="space-y-2.5">
            <li v-for="cat in categories" :key="cat">
              <label class="flex items-center cursor-pointer group">
                <input type="radio" :value="cat" v-model="selectedCategory" class="w-4 h-4 text-blue-500 focus:ring-blue-500 border-gray-300 rounded-full" />
                <span class="ml-3 text-sm" :class="selectedCategory === cat ? 'text-blue-500 font-semibold' : 'text-gray-600 group-hover:text-gray-900'">{{ cat }}</span>
              </label>
            </li>
          </ul>
        </div>
        ...
        <div class="mb-8 pl-1">
          ...
          <ul class="space-y-2.5">
            <li>
              <label class="flex items-center cursor-pointer group">
                <input type="radio" value="All" v-model="selectedBrand" class="w-4 h-4 text-blue-500 border-gray-300 rounded-full" />
                <span 
                  class="ml-3 text-sm"
            ...
            <li v-for="brand in allBrands" :key="brand">
              <label class="flex items-center cursor-pointer group">
                <input type="radio" :value="brand" v-model="selectedBrand" class="w-4 h-4 text-blue-500 border-gray-300 rounded-full" />
                <span class="ml-3 text-sm"
                  :class="selectedBrand === brand ? 'text-blue-500 font-semibold' : 'text-gray-600 group-hover:text-gray-900'">
                  {{ brand }}
                </span>
              </label>
            </li>
        ...
        <div class="mb-8 pl-1">
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-sm font-bold text-blue-500">₱{{ maxPrice }}</span>
          </div>
          <input 
            type="range" 
            min="0" 
        ...
        <div class="pl-1">
          ...
          <div class="flex flex-wrap gap-2">
            <button 
              ...
              :key="tag"
              @click="toggleTag(tag)"
              :class="['px-3 py-1.5 text-xs font-semibold rounded-full transition-colors border', selectedTags.includes(tag) ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-300']"
            >
              {{ tag }}
            </button>
          </div>
      ...
      <div class="hidden lg:block bg-white border-b border-gray-200 py-8 px-4 sm:px-6 lg:px-8 mb-5">
          ...
          >
            ...
            <div class="p-6 flex flex-col flex-1 relative bg-white">
              <div class="flex justify-between items-start gap-4 mb-2">
                <div class="flex-1">
                  <div class="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-1">{{ product.category }}</div>
        ...
        <div v-else class="space-y-4">
          ...
          >
            ...
            <div class="p-6 flex-1 flex flex-col justify-center relative">
              <div class="text-xs font-bold text-blue-500 uppercase tracking-widest mb-1 opacity-80">{{ product.category }}</div>
        ...
        <div v-if="filteredProducts.length === 0" class="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm mt-0">
          ...
          <button @click="selectedCategory = 'All'; maxPrice = 2000; selectedTags = []; searchQuery = ''" class="mt-6 text-blue-500 font-semibold hover:underline">Clear all filters</button>
        </div>
    ...
    >
      <div v-if="quickViewProduct" class="fixed inset-y-0 right-0 w-full sm:w-[500px] bg-white shadow-2xl z-[70] flex flex-col overflow-y-auto border-l border-gray-100">
        ...
        <div class="p-6 pt-0 flex flex-col flex-1 relative bg-white">
          <div class="flex justify-between items-start gap-4 mb-2">
            <div class="flex-1">
                <div class="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-1">{{ quickViewProduct.category }}</div>
          ...
          <div class="mt-auto">
            <div v-if="quickViewProduct.inStock" class="flex flex-col gap-5">
              ...
              <button @click="addToCart" class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center hover:scale-[1.02] active:scale-95 text-lg">
                <ShoppingBag class="w-6 h-6 mr-3" /> Add to Cart
              </button>
            </div>
            <div v-else class="text-center p-5 bg-red-50 text-red-600 font-bold rounded-xl border border-red-100">
            ...
            <div class="text-center border-t border-gray-100 pt-6">
              <router-link :to="`/product/${quickViewProduct.id}`" class="text-blue-500 font-bold hover:underline inline-flex items-center group">
                View Full Product Details <ArrowRight class="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
              </router-link>
            </div>
          </div>
